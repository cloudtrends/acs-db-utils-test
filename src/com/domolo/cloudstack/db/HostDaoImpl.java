...
public class HostDaoImpl extends GenericDaoBase<HostVO, Long> implements HostDao { //FIXME: , ExternalIdDao {
    ...
    //tcf protected SearchBuilder<HostTransferMapVO> HostTransferSearch;
    //tcf protected SearchBuilder<ClusterVO> ClusterManagedSearch;
    ...
    private List<Long> listAllClusters() {
        ...
        SearchCriteria<Long> sc =null;// AllClustersSearch.create();
        sc.setParameters("managed", Managed.ManagedState.Managed);

        List<Long> clusters =null;//tcf _clusterDao.customSearch(sc, null);
        return clusters;
    }
    ...
    public List<HostVO> listByHostTag(Host.Type type, Long clusterId, Long podId, long dcId, String hostTag) {
        ...
        //tcf
        //SearchBuilder<HostTagVO> hostTagSearch = null;//tcf _hostTagsDao.createSearchBuilder();
    ...
    public List<HostVO> listAllUpAndEnabledNonHAHosts(Type type, Long clusterId, Long podId, long dcId, String haTag) {
        ...
        if (haTag != null && !haTag.isEmpty()) {
            //tcf
            //hostTagSearch = null;//tcf //_hostTagsDao.createSearchBuilder();
    ...
    public void loadDetails(HostVO host) {
        Map<String, String> details = null;//tcf _detailsDao.findDetails(host.getId());
        host.setDetails(details);
    }

    @Override
    public void loadHostTags(HostVO host) {
        List<String> hostTags = null; //tcf _hostTagsDao.gethostTags(host.getId());
    ...
    public void saveDetails(HostVO host) {
       ...
       //tcf _detailsDao.persist(host.getId(), details);
    }
    ...
    protected void saveHostTags(HostVO host) {
      ...
      //tcf  _hostTagsDao.persist(host.getId(), hostTags);
    }
    ...
    @Override
    public boolean updateState(Status oldStatus, Status.Event event, Status newStatus, Host vo, Object data) {
        HostVO host = findById(vo.getId());
        if (host == null) {
            if (event == Status.Event.Remove && newStatus == Status.Removed) {
        ...
        if (newStatus.updateManagementServer()) {
            ...
            if (event.equals(Status.Event.Ping) || event.equals(Status.Event.AgentConnected)) {
                ub.set(host, _pingTimeAttr, System.currentTimeMillis() >> 10);
            }
        }
        if (event.equals(Status.Event.ManagementServerDown)) {
